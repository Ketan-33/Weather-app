    ...
    private void getTodayWeatherInfo(String name) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url.getLink(), null, response -> {
            try {
                ...
                // Extracting the weather condition details
                ...
                // Extracting temperature and other details from the "main" object
                temperature = String.valueOf(Math.round(response.getJSONObject("main").getDouble("temp") - 273.15));
                min_temperature = String.format("%.0f", response.getJSONObject("main").getDouble("temp_min") - 273.15);
                max_temperature = String.format("%.0f", response.getJSONObject("main").getDouble("temp_max") - 273.15);
                ...
                // Extracting sunrise and sunset times from the "sys" object
        ...
        requestQueue.add(jsonObjectRequest);
        Log.i("json_req", "Weather info loaded");
    }
